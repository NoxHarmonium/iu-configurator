...
use crate::utils::time::{mmss_to_secs, secs_to_mmss};
...
pub fn ConfigPage() -> impl IntoView {
    ...
    let poll_ms = Signal::derive(move || {
        setup_res
            .get()
            .and_then(Result::ok)
            .map_or(5000, |s: ClientSetupInfo| s.poll_interval_ms)
    });
                    ...
                    for zid in &zone_ids {
                        if let Some(zs) = s.zones.get_mut(zid.as_str()) {
                            ...
                            zs.morning_secs = mmss_to_secs(
                                morning_snap
                                    .get(zid)
                                    .map_or("00:00", std::string::String::as_str),
                            );
    ...
    view! {
        <div class="page">
            ...
            <Transition fallback=|| view! { <p class="loading">"Loading configuration…"</p> }>
                {move || {
                    schedule_res.get().map(|result| if let Err(e) = result { view! {
